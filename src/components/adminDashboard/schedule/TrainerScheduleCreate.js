...
const TrainerScheduleCreate = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [trainersRes, usersRes, groupsRes] = await Promise.all([
          axios.get('https://localhost:7054/api/Admin/trainers', { headers }),
          axios.get('https://localhost:7054/api/Admin/users', { headers }),
          axios.get('https://localhost:7054/api/Group/all-groups', { headers })
        ]);
        setTrainers(trainersRes.data);
        setUsers(usersRes.data);
        setGroups(groupsRes.data);
